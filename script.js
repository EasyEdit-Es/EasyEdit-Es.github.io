...
if (cdWrap) {
  const target = new Date('2026-06-30T17:00:00');
...
// ══════════════════════════════════════════
// SOUND SYSTEM — Sonidos sintetizados Web Audio API
// ══════════════════════════════════════════
let audioCtx = null;
...
function playNavClick() {
  ...
  try {
    ...
    [0, 0.05, 0.1].forEach((t, i) => {
      ...
      gain.gain.setValueAtTime(0.025, ctx.currentTime + t);
...
if (soundBtn) {
  soundBtn.addEventListener('click', () => {
    ...
    // Play confirmation beep when enabling
    if (soundEnabled) {
      try {
        const ctx = getAudioCtx();
...
document.querySelectorAll('a, button:not(#sound-toggle), .map-tab').forEach(el => {
  el.addEventListener('mouseenter', playHover);
  el.addEventListener('click', () => {
    if (el.matches('.map-tab')) playNavClick();
    else playClick();
  });
});
